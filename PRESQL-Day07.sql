...
WHERE word ~ '^[say](.*)'; -- ^ ilk karakter dedigi zaman bu işaret kullanılır

--Son karakteri 'm', 'a' veya 'f' olan "word" değerlerini çağırın.
SELECT word FROM words
WHERE word ~ '(.*)[maf]$'; -- son karakter isterse $ işareti kullanılır
...
SELECT word FROM words
WHERE word ~ '^[d-t].l(.*)'; -- .* herhangi bir harfle devam edebilir

--İlk karakteri 'd' den 't' ye olan,  herhangi iki karakter ile devam edip dördüncü karakteri 'e' olan "word" değerlerini çağırın.

SELECT word FROM words
WHERE word ~ '^[d-t]..e(.*)'; -- her . underscored aki _ gibidir herhangi bir harfi temsil eder
...
SELECT name FROM workers
WHERE name ~ '^[^JBE]' OR name ~ '[^rt]$'; -- ^ köşeli paranteze bu işareti koyunca olumsuzluk katıyor


--Herhangi bir yerinde 'a' yada 'r' bulunan "name" değerlerini çağırın.

SELECT name FROM workers
WHERE name ~ '[ar]'; -- böyle köşeli prantez ile yazarsak a ve r olan butun name leri bana getirir
-- ama direk ar yazarsak name de bitişik ar yi arar

SELECT name FROM workers
WHERE name ~ 'ar'; -- sadece mark stone getirir
