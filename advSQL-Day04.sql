...
-- halil hocanın çalışması fibonacci için
do $$
	declare 
	n integer:=8;
	counter integer:=0;
	...
	loop
		exit when counter=n;
		if n<0 then 
			raise notice 'hata negatif sayı giremezsiniz sma ben düzelttim :)';
			n:=n*(-1);
		elseif n=0 then 
			raise notice 'Bu da olmadıki';
			exit;
		end if;
		raise notice '%. Sayı: %',counter,fib;
		counter:=counter+1;
		fib:=ilkDeg + sonDeg;
		ilkDeg:= sonDeg;
		sonDeg:= fib;
...
--cumali hocanın çalışması fibonacci için
DO
$$
DECLARE
	n INTEGER = -1;
...
begin
	  ...
	  raise notice '% (% dakika)', film_lenght.title, film_lenght.lenght;
end loop;	  
