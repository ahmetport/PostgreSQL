...
declare                              --dinamik olarak data type larını db den alabiliriz. 
	  ...
	  film_length film.lenght%type;  --film tablosundaki column data type i degisirse buradaki kod onu otomatik gorup degisecek dinamik koddur. 
...
begin
	 ...
	 select type
	 from film
	 into film_type  -- embed etmek yani degişkenimizin içine başka bir bir data atamak
	 where title='Shrek';
	 raise notice 'Adı Shrek olan filmin turu %', film_type;
	 
end $$;	 



--yukardaki örnegin aynısı sadece bu sefer daha dinamik hale getirdik 
-- harf karakterlere ve yazılımlara çok dikkat etmemiz lazım
do $$
declare                              
								     							    
      film_type film.type%type;   
	  film_length film.lenght%type;  
      film_title film.title%type := 'Kuzuların Sessizligi';  
begin
     --task: Name i Kuzuların Sessizligi olan filmin type getirelim
...
declare
   ...
   selected_actor actor%rowtype; -- butun row type selected_actor içine attık onu aşagıda embed edecegiz

begin
   SELECT * 
   FROM actor
   into selected_actor  --embed ettim
   where id = 1;
   
       raise notice 'the actor name is :% %',      -- iki tane yer tutucu koydum biri first name digeri last name için
